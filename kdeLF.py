...
class KdeLF():
    ...
    def load_sample(self):   
        ...
        kde.params.weighting, kde.params.absolute_magnitude = self.weighting, self.absolute_magnitude             
        if self.weighting is True:
            ...
            kde.params.weight,kde.params.nw=KdeLF.weight,self.Neff                    
        else:
            self.Neff=KdeLF.ndata       
        
        KdeLF.yL=KdeLF.lum-self.f_lim(KdeLF.red)
        KdeLF.xz=np.log( (KdeLF.red-KdeLF.z1)/(KdeLF.z2-KdeLF.red) )
        kde.params.xz,kde.params.ylum,kde.params.red=KdeLF.xz,KdeLF.yL,KdeLF.red
        ...
        kde.f2py_value(KdeLF.ndata,input_params,Number_of_threads_used)         
        kde.params.small_sample,kde.params.adaptive = self.small_sample,self.adaptive 
       
        rato=kde.initialize(self.epson)
        rato=rato*100
        print('epson',self.epson,'rato','%.6f' %rato,'%')
        
        limx=np.linspace(np.min(KdeLF.red),KdeLF.z2,50)
        limy=self.f_lim(limx)             # &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
        kde.params.limx,kde.params.limy=limx,limy        
...
#        plt.show()
        ...
        kde.params.unbounded_lum=self.unbounded_lum
...
#    from scipy.integrate import dblquad
    ...
    def prepare_KDE(self,theta):                     #$$$$$$$$$$$$$$$ small_sample not finished
        ...
        if self.adaptive is True:
            ...
            kde.params.hi, kde.params.h10, kde.params.h20, kde.params.beta = KdeLF.hi, h10, h20, beta            
        else:
            h1,h2=theta
            kde.params.h1, kde.params.h2 = h1, h2           
        return 
    ...
    def log10phi(self,z,L,theta):      #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$           
        ...
        if self.adaptive:
            h10,h20,beta=theta
            kde.params.h10,kde.params.h20,kde.params.beta=h10,h20,beta    
        else:
            h1,h2=theta
            kde.params.h1,kde.params.h2=h1,h2            
        result=kde.prob(z,L)*self.Neff/(dvdz*self.Omega)
...
#        x=np.log( (z-z1)/(z2-z) )
#        y=L-f_lim(z)    
#        return kde.f_ref_f2py(x,y) * ( 1/(z-z1) + 1/(z2-z) )
    
    ##################################################################################################################################
    def _2lnlike(self,para):
        h1,h2=para     
        #print(para)                                          
        result= -2 * kde.lnlike(h1,h2)                
        return result
        

    def _2lnlike_1d(self,para):
        result=-2*kde.lnlike_1d(para)
        return result
        

    def _2lnlike_1da(self,para):
        h_0,beta=para        
        result=-2*kde.lnlike_1da(h_0,beta)
        return result  
    ...
    def _2lnlike_adaptive(self,para):
        ...
        if self.set_beta_fixed:
            ...
            h10,h20,beta=para    
        result= -2 * kde.lnlike_ada(h10,h20,beta)
        return result 
    ...
    def get_h_1da(self,h_pilot,bounds_h0_beta):
        ...
        kde.params.hi, kde.params.f_pilot, kde.params.set_beta_fixed=KdeLF.hi, KdeLF.f_pilot,self.set_beta_fixed 
